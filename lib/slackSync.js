...
async function syncChannelHistory(viewer, teamId, channelId, token, opts = {}) {
  // DM / group-DM messages are tagged with the user who synced them so they
  // stay private to that user. Channel messages (public/private) are shared
  // → synced_by_user_id stays NULL.
  const isDm = opts.kind === "im" || opts.kind === "mpim";
  const syncedBy = isDm ? (opts.ownerUserId || null) : null;
    ...
    for (const m of messages) {
      ...
      await pool.query(
        `INSERT INTO slack_messages
           (team_id, channel_id, ts, thread_ts, slack_user_id, user_name, text_body, subtype, reactions, file_ids, permalink, synced_by_user_id, ingested_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, $10::text[], $11, $12, NOW())
         ON CONFLICT (team_id, channel_id, ts) DO UPDATE SET
           text_body = EXCLUDED.text_body,
           reactions = EXCLUDED.reactions,
           synced_by_user_id = COALESCE(slack_messages.synced_by_user_id, EXCLUDED.synced_by_user_id),
           ingested_at = NOW()`,
        [
          teamId,
          channelId,
          ...
          syncedBy, // owner for DMs/group-DMs; NULL for shared channels
        ],
      );
...
async function syncWorkspaceBot(ws) {
  ...
  for (const ch of channels) {
    try {
      const r = await syncChannelHistory(viewer, ws.team_id, ch.id, ws.bot_token, { kind: ch.kind, ownerUserId: null });
...
async function syncUser(tiUserId) {
  ...
  for (const ch of channels) {
    try {
      const r = await syncChannelHistory(viewer, conn.team_id, ch.id, conn.user_token, { kind: ch.kind, ownerUserId: tiUserId });
      totalIngested += r.ingested;
    } catch (err) {
