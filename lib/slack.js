...
// Local search across transcribed voice notes. Slack's own search index
// doesn't know about our transcripts, so this ALWAYS runs (even alongside
// live search) to make voice notes findable. Phase 5.CV.
...
async function searchMessages(tiUserId, query, { limit = 15 } = {}) {
  ...
  if (conn) {
    try {
      ...
      // them). Dedup by channel+ts, voice notes first since they're the
      // content the user otherwise can't find any other way.
      let voiceHits = [];
      try { voiceHits = await searchTranscriptsLocal(localTerms, lim); } catch (_) {}
