...
async function status(userId) {
  // Phase 5.CH — combined status. Shape matches what settings.html
  // (post Nexa-port) reads: data.configured, data.workspace.{installed,
  // name, installedAt, id}, data.user.{connected, name, email, id}.
  // Old snake_case fields kept alongside for any legacy callers.
  const ws = await getWorkspace();
  const ut = userId ? await getUserToken(userId) : null;
  const workspace = {
    installed: !!ws,
    id:          ws?.team_id || null,
    name:        ws?.team_name || null,
    installedAt: ws?.installed_at || null,
    botUserId:   ws?.bot_user_id || null,
    // legacy fields for older callers
    team_id:     ws?.team_id || null,
    team_name:   ws?.team_name || null,
    installed_at: ws?.installed_at || null,
  };
  const user = {
    connected: !!ut,
    id:    ut?.slack_user_id || null,
    name:  ut?.slack_user_name || null,
    email: ut?.slack_email || null,
    teamId: ut?.team_id || null,
    connectedAt: ut?.connected_at || null,
  };
  return {
    configured: isConfigured(),
    workspace,
    user,
    // legacy top-level fields for older callers (chat-mode lib reads these)
    user_connected: !!ut,
    user_connection: ut ? {
      team_id: ut.team_id,
...
// ---------------------------------------------------------------------------
// Operator-stripped search terms (shared by transcript + local search).
function searchTerms(query) {
  return String(query || "")
    .replace(/\b(from|in|has|before|after|with):\S+/gi, "")
    .split(/\s+/)
    .filter(Boolean);
}

// Shape a voice-note row into a search result. Farsi transcripts are
// flagged so Delta never presents an auto-transcript as hard fact.
function voiceRowToResult(row) {
  const lang = row.transcript_lang || "en";
  const body = row.transcript_en || row.transcript || "";
  const flag = lang === "fa" ? " (Farsi — auto-transcribed, may be imperfect)" : "";
  return {
    ...
    text: `🎙️ Voice note${flag}: ${body}`,
    is_voice_note: true,
    transcript_lang: lang,
    transcript_original: row.transcript || null,
...
const TRANSCRIPT_SELECT = `
  SELECT m.team_id, m.channel_id, m.ts, m.thread_ts, m.slack_user_id,
         m.user_name, m.transcript, m.transcript_en, m.transcript_lang, m.permalink,
         c.name AS channel_name, c.kind AS channel_kind,
         u.real_name AS resolved_user
    FROM slack_messages m
    LEFT JOIN slack_channels c ON c.team_id = m.team_id AND c.channel_id = m.channel_id
    LEFT JOIN slack_users u    ON u.team_id = m.team_id AND u.slack_user_id = m.slack_user_id`;

// Content-keyword search across transcribed voice notes. Slack's own
// search index doesn't know about our transcripts, so this ALWAYS runs
// (even alongside live search) to make voice notes findable. Phase 5.CV.
async function searchTranscriptsLocal(terms, lim, ownerUserId) {
  if (!terms || !terms.length) return [];
  const conds = terms
    .map((_, i) => `(lower(m.transcript) LIKE '%' || lower($${i + 1}) || '%' OR lower(coalesce(m.transcript_en,'')) LIKE '%' || lower($${i + 1}) || '%')`)
    .join(" AND ");
  // Privacy scope: shared channel rows (synced_by_user_id IS NULL) OR the
  // requesting user's OWN DMs. Never another user's DMs.
  const params = [...terms, ownerUserId || null];
  const ownerParam = `$${params.length}`;
  const r = await pool.query(
    `${TRANSCRIPT_SELECT}
      WHERE m.transcript IS NOT NULL AND (${conds})
        AND (m.synced_by_user_id IS NULL OR m.synced_by_user_id = ${ownerParam})
      ORDER BY m.ts DESC
      LIMIT ${lim}`,
    params,
  );
  return r.rows.map(voiceRowToResult);
}

// Voice notes within specific channels — used to surface a DM/channel's
// voice notes when the user asks about a *conversation* (e.g.
// "summary of my conversation with Anet"). Such queries are operator-only
// (with:@anet) with no content keywords, so the keyword search above
// finds nothing; but Slack's live search tells us which channel the
// conversation is in, and we pull that channel's transcribed clips here.
// Phase 5.CV.1. Optional content `terms` further narrow within the
// channels. Returns newest-first.
async function searchTranscriptsByChannels(channelIds, terms, lim, ownerUserId) {
  if (!channelIds || !channelIds.length) return [];
  const params = [channelIds];
  let where = `m.transcript IS NOT NULL AND m.channel_id = ANY($1)`;
  (terms || []).forEach((t, i) => {
    where += ` AND (lower(m.transcript) LIKE '%' || lower($${i + 2}) || '%' OR lower(coalesce(m.transcript_en,'')) LIKE '%' || lower($${i + 2}) || '%')`;
    params.push(t);
  });
  // Privacy scope: shared channel rows OR the requesting user's OWN DMs.
  params.push(ownerUserId || null);
  where += ` AND (m.synced_by_user_id IS NULL OR m.synced_by_user_id = $${params.length})`;
  const r = await pool.query(
    `${TRANSCRIPT_SELECT}
      WHERE ${where}
      ORDER BY m.ts DESC
...
async function searchMessages(tiUserId, query, { limit = 15 } = {}) {
  ...
  const conn = await getUserToken(tiUserId);
  const localTerms = searchTerms(query);
  ...
  if (conn) {
    try {
      ...
      const liveResults = matches.map((m) => {
      ...
      // Merge in voice-note transcript hits (Slack's index can't see
      // them). Two sources, since voice notes surface differently:
      //   • by keyword — "voice note about passwords" (content terms)
      //   • by channel — "summary of my conversation with Anet" is an
      //     operator-only query (with:@anet, no keywords); the live
      //     results above tell us the DM/channel, so we pull THAT
      //     channel's transcribed clips even with zero keywords.
      let voiceHits = [];
      try {
        const channelIds = [...new Set(liveResults.map((x) => x.channel_id).filter(Boolean))];
        const [byTerm, byChannel] = await Promise.all([
          searchTranscriptsLocal(localTerms, lim, tiUserId),
          searchTranscriptsByChannels(channelIds, localTerms, lim, tiUserId),
        ]);
        const seenV = new Set();
        voiceHits = [...byChannel, ...byTerm].filter((v) => {
          const k = `${v.channel_id}|${v.ts}`;
          if (seenV.has(k)) return false;
          seenV.add(k);
          return true;
        });
      } catch (_) {}
      const seen = new Set(liveResults.map((x) => `${x.channel_id}|${x.ts}`));
      const merged = [...voiceHits.filter((v) => !seen.has(`${v.channel_id}|${v.ts}`)), ...liveResults];

      return {
        ok: true,
        source: voiceHits.length ? "live+voice" : "live",
        query,
        count: merged.length,
        results: merged,
      };
  ...
  // Each term must appear in the message text OR in a voice-note
  // transcript (original or English translation). Phase 5.CV.
  ...
  const conds = terms
    .map((_, i) => `(lower(m.text_body) LIKE '%' || lower($${i + 1}) || '%' OR lower(coalesce(m.transcript,'')) LIKE '%' || lower($${i + 1}) || '%' OR lower(coalesce(m.transcript_en,'')) LIKE '%' || lower($${i + 1}) || '%')`)
    .join(" AND ");
  // Privacy scope: shared channel rows (synced_by_user_id IS NULL) OR the
  // requesting user's OWN DMs — never another user's DMs (the leak this fixes).
  const params = [...terms, tiUserId || null];
  const ownerParam = `$${params.length}`;
  const r = await pool.query(
    `SELECT m.team_id, m.channel_id, m.ts, m.thread_ts, m.slack_user_id,
            m.user_name, m.text_body, m.transcript, m.transcript_en, m.transcript_lang, m.permalink,
            c.name AS channel_name, c.kind AS channel_kind,
            u.real_name AS resolved_user
       FROM slack_messages m
  ...
  return {
    ...
    results: r.rows.map((row) => {
      // If this message is a voice note (has a transcript), surface the
      // spoken content rather than the empty text_body.
      if (row.transcript) return voiceRowToResult(row);
      return {
        channel_id: row.channel_id,
        channel_name: row.channel_name || row.channel_id,
        channel_kind: row.channel_kind,
