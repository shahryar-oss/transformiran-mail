...
//
// Claude rejects any single image over ~10 MB, so we render at a modest
// scale and step DOWN if a page comes out too big (a 2× A4 PNG can be
// ~11 MB). Prefer the highest scale whose every page is comfortably under
// the limit; at the smallest scale, keep pages under the hard cap and drop
// any that are still oversized (some pages beat erroring out entirely).
async function renderPdfPages(buf, { maxPages = 5 } = {}) {
  if (!pdfParse || typeof pdfParse.PDFParse !== "function") return [];
  const HARD_LIMIT = 9_500_000;   // stay safely under Claude's ~10 MB/image
  const TARGET     = 7_500_000;   // prefer this much headroom
  const renderAt = async (scale) => {
    ...
    try {
      ...
      return pages.map((p) => {
        ...
        return { mediaType: "image/png", dataB64: b64, bytes: Math.floor((b64.length * 3) / 4) };
      }).filter((x) => x.dataB64);
    } finally {
      try { if (parser && parser.destroy) await parser.destroy(); } catch (_) {}
    }
  ...
  try {
    for (const scale of [1.5, 1]) {
      const pages = await renderAt(scale);
      if (!pages.length) continue;
      if (pages.every((p) => p.bytes <= TARGET)) {
        return pages.map(({ mediaType, dataB64 }) => ({ mediaType, dataB64 }));
      }
      if (scale === 1) {
        return pages.filter((p) => p.bytes <= HARD_LIMIT)
                    .map(({ mediaType, dataB64 }) => ({ mediaType, dataB64 }));
      }
    }
    return [];
  } catch (_) {
