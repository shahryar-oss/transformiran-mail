...
async function initSchema() {
  ...
  // Enable pg_trgm up front so the trigram (gin_trgm_ops) indexes below
  // can be created. Without this, those CREATE INDEX statements throw
  // "operator class gin_trgm_ops does not exist", and because initSchema
  // is awaited by dbReady, a single throw rejects the whole chain and
  // EVERY background worker (backfill, inbox-cache, snooze, slack-sync…)
  // fails to start. (Incident 2026-05-27: this exact failure left Pia's
  // backfill stuck because the worker never booted.) Wrapped in catch:
  // if the DB role lacks CREATE EXTENSION rights we still proceed — the
  // individual trigram indexes each have their own non-trgm fallback.
  await pool.query(`CREATE EXTENSION IF NOT EXISTS pg_trgm;`).catch((err) => {
    console.warn("[db] could not enable pg_trgm extension (trigram indexes will fall back):", err.message);
  });
  ...
  // Snooze defaults — five time-of-day shortcuts used by snooze + schedule-send.
  await pool.query(`
    ALTER TABLE users
      ADD COLUMN IF NOT EXISTS snooze_defaults JSONB
      DEFAULT '{"morning":"08:00","afternoon":"13:00","evening":"18:00","later":"+3h","nextWeek":"MON 08:00"}'::JSONB;
  `);

  // Compose preferences — Markdown shortcuts + undo-send delay.
  await pool.query(`
    ALTER TABLE users
      ADD COLUMN IF NOT EXISTS compose_settings JSONB
      DEFAULT '{
        "undoSendSeconds": 10,
  ...
  // Calendar preferences — focus blocks (when Delta must NOT schedule),
  // meeting buffer, meeting prep toggle, default meeting link, watched calendars.
  await pool.query(`
    ALTER TABLE users
  ...
  // Classifier sensitivity — small integer 1..5. 3 = balanced (default).
  await pool.query(`
    ALTER TABLE users
      ADD COLUMN IF NOT EXISTS classifier_sensitivity SMALLINT DEFAULT 3;
  `);

  // Appearance preferences — synced across devices via this column rather
  // than localStorage so the user picks once and sees the same theme
  // everywhere. Applied by every page via shared appearance.js boot script.
  await pool.query(`
    ALTER TABLE users
  ...
  // Notification preferences — what should fire a browser notification,
  // when, and from whom. mode = 'smart' | 'custom' | 'off'.
  await pool.query(`
    ALTER TABLE users
  ...
  // Smart folders — saved searches pinned to the left rail.
  await pool.query(`
  ...
  `);
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_smart_folders_user
      ON smart_folders(user_id, position);
  `);

  // Snippets — text-expansion templates for the composer.
  await pool.query(`
  ...
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_snippets_user
      ON snippets(user_id, last_used_at DESC NULLS LAST, created_at DESC);
  `);

  // Bug reports — submitted via Settings → Support.
  await pool.query(`
  ...
  // ===================================================================
  // ADMIN CONSOLE TELEMETRY (Phase: admin dashboard).
  // ===================================================================

  // Block flag — a blocked user is bounced at auth.
  await pool.query(`
    ALTER TABLE users
  ...
  // delta_usage — one row per Delta chat/voice/bridge call, with token
  // counts + computed USD cost, so the admin console can show Delta spend
  // per user / model / surface over time.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS delta_usage (
      id          BIGSERIAL PRIMARY KEY,
      user_id     BIGINT REFERENCES users(id) ON DELETE CASCADE,
      surface     TEXT,                  -- 'chat' | 'voice' | 'bridge'
      model       TEXT,
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_delta_usage_user_time ON delta_usage(user_id, created_at DESC);`);

  // email_sends — one row per email the user sends through the platform.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS email_sends (
      ...
      to_email    TEXT,
      subject     TEXT,
      kind        TEXT,                  -- 'reply' | 'reply-all' | 'compose' | 'forward'
      thread_id   TEXT,
      created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_email_sends_user_time ON email_sends(user_id, created_at DESC);`);

  // user_sessions — heartbeat-based activity windows for "time on app".
  // The client pings while the tab is open; a ping > IDLE_GAP after the
  // last one starts a new session row. Duration ≈ last_ping - started_at.
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_user_sessions_user_time ON user_sessions(user_id, last_ping_at DESC);`);

  // delta_chat_messages — persisted Delta conversation turns for quality
  // monitoring in the admin console (review what Delta said per user).
  await pool.query(`
    CREATE TABLE IF NOT EXISTS delta_chat_messages (
      id          BIGSERIAL PRIMARY KEY,
      user_id     BIGINT REFERENCES users(id) ON DELETE CASCADE,
      role        TEXT NOT NULL,         -- 'user' | 'assistant'
      content     TEXT,
      surface     TEXT,                  -- 'chat' | 'voice'
      model       TEXT,
      created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_delta_chat_user_time ON delta_chat_messages(user_id, created_at DESC);`);
  ...
  // ====================================================================
  // Phase 5.BR — Slack integration.
  // Two-token model:
  //   • slack_workspaces — one row per Slack workspace. Bot token reads
  //     shared channels + files. Installed by an admin via the
  //     workspace-install OAuth flow.
  //   • slack_user_tokens — one row per (TI user, Slack user) pair.
  //     User token reads DMs + does workspace-wide search.
  // ====================================================================
  await pool.query(`
    CREATE TABLE IF NOT EXISTS slack_workspaces (
      ...
      scope          TEXT,                  -- comma-list of granted bot scopes
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS slack_user_tokens (
      ...
      scope           TEXT,                 -- comma-list of granted user scopes
      connected_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      PRIMARY KEY (user_id, team_id)
    );
  `);
  // Short-lived OAuth state tokens — we generate one per OAuth start and
  // verify it on callback to prevent CSRF / replay. 10 min TTL.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS slack_oauth_state (
      state       TEXT PRIMARY KEY,
      user_id     BIGINT REFERENCES users(id) ON DELETE CASCADE,
      flow        TEXT NOT NULL,            -- 'install' or 'connect'
      created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
  `);

  // ====================================================================
  // Phase 5.BS — Slack content sync (per-workspace channels + DMs).
  // The bot can paginate any channel it's a member of. Each TI user's
  // personal token can paginate any channel/DM they have access to.
  // We store: a list of channels + a per-(channel,viewer) cursor, plus
  // the actual messages. "viewer" is either the bot or a TI user id.
  // ====================================================================
  await pool.query(`
    CREATE TABLE IF NOT EXISTS slack_channels (
      ...
      name           TEXT,
      kind           TEXT,            -- public_channel | private_channel | im | mpim
      is_archived    BOOLEAN NOT NULL DEFAULT FALSE,
      created        BIGINT,          -- unix seconds (Slack 'created' field)
      num_members    INT,
      topic          TEXT,
      purpose        TEXT,
      updated_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS slack_sync_cursors (
      viewer         TEXT NOT NULL,   -- 'bot:<team_id>'  OR  'user:<ti_user_id>'
      team_id        TEXT NOT NULL,
      channel_id     TEXT NOT NULL,
      latest_ts      TEXT,            -- newest Slack ts we've ingested
      last_sync_at   TIMESTAMPTZ,
      PRIMARY KEY (viewer, team_id, channel_id)
    );
  `);
  // Slack message timestamps are double-precision strings — keep as TEXT
  // so we don't lose precision. Composite PK on (team_id, channel_id, ts)
  // means each message stored once even if multiple viewers see it.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS slack_messages (
      team_id        TEXT NOT NULL,
      channel_id     TEXT NOT NULL,
      ts             TEXT NOT NULL,         -- Slack's message ts (also the message id within a channel)
      thread_ts      TEXT,                   -- if part of a threaded reply
      slack_user_id  TEXT,
      user_name      TEXT,                   -- denormalised at write time for fast renders
      text_body      TEXT,
      subtype        TEXT,                   -- e.g. 'channel_join', 'bot_message', NULL = normal
      reactions      JSONB,                  -- [{name, count, users[]}]
      file_ids       TEXT[],                 -- file IDs attached to this message
      permalink      TEXT,
      ingested_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      PRIMARY KEY (team_id, channel_id, ts)
    );
  `);
  // Phase 5.CV — voice-note transcripts. Stored on the message row (NOT
  // text_body, which the sync upsert overwrites) so they survive re-sync.
  //   transcript        — verbatim transcription (original language)
  //   transcript_en     — English translation (only when non-English)
  //   transcript_lang   — detected language ("en" / "fa")
  //   transcript_status — NULL = not yet processed; 'done' / 'no_audio' /
  //                       'empty' / 'error'
  await pool.query(`
  ...
  `);
  // Per-user DM privacy: tag DM / group-DM (im / mpim) messages with the
  // user whose token synced them. NULL = shared channel content (public /
  // private channels, synced by the bot or any member). Local + transcript
  // search returns shared rows OR the requesting user's OWN DMs only — so
  // one person's DMs never surface in another person's Delta.
  await pool.query(`
    ALTER TABLE slack_messages
      ADD COLUMN IF NOT EXISTS synced_by_user_id BIGINT;
  `);
  // One-time cleanup: purge legacy DM / group-DM rows that were ingested
  // BEFORE per-user attribution existed (they have no owner, so they'd be
  // treated as "shared" and leak across users). They re-sync attributed on
  // the next cycle. Idempotent: once gone, this deletes 0 rows.
  await pool.query(`
    DELETE FROM slack_messages m
     WHERE m.synced_by_user_id IS NULL
       AND EXISTS (
         SELECT 1 FROM slack_channels c
          WHERE c.team_id = m.team_id AND c.channel_id = m.channel_id
            AND c.kind IN ('im', 'mpim')
       );
  `);
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_slack_messages_owner
      ON slack_messages(synced_by_user_id) WHERE synced_by_user_id IS NOT NULL;
  `);
  // Fast lookup of voice notes still needing transcription.
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_slack_messages_transcript_pending
      ON slack_messages(team_id) WHERE transcript_status IS NULL;
  `);
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_slack_messages_team_user
      ON slack_messages(team_id, slack_user_id);
  `);
  await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_slack_messages_text_trgm
      ON slack_messages USING gin (lower(text_body) gin_trgm_ops);
  `).catch(async () => {
    // pg_trgm unavailable — fall back to a plain btree index so init
    // doesn't reject (which would take down every background worker).
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_slack_messages_text
        ON slack_messages(team_id, lower(text_body));
    `).catch(() => {});
  });
  await pool.query(`
    CREATE TABLE IF NOT EXISTS slack_files (
      ...
      channel_id     TEXT,                   -- first channel we saw it in (best effort)
      filename       TEXT,
      title          TEXT,
      mimetype       TEXT,
      filetype       TEXT,
      size_bytes     BIGINT,
      url_private    TEXT,                   -- requires Authorization header to fetch
      url_private_download TEXT,
      slack_user_id  TEXT,                   -- uploader
      created        BIGINT,                 -- unix seconds
      ingested_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      PRIMARY KEY (team_id, file_id)
    );
  `);
  // Slack user directory cache — speeds up name resolution at render time.
  await pool.query(`
