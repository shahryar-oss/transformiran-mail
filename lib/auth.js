...
async function attachUser(req, res, next) {
  ...
  try {
    const r = await pool.query(
      `SELECT id, email, display_name, picture_url, welcomed_at, preferred_model,
              signature_mode, default_send_as, blocked_at
...
function requireAuth(req, res, next) {
  ...
  // Blocked users (set from the admin console) are bounced everywhere.
  if (req.user.blocked_at) {
    clearSession(res);
    if (req.accepts("html") && req.method === "GET") {
      return res.status(403).send("<h1>Access suspended</h1><p>Your access to Delta Mail has been suspended. Please contact your administrator.</p>");
    }
