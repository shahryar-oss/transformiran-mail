...
Rules:
- "top_replies" should have 3-8 entries: emails that genuinely need ${firstName}'s reply today, ranked by priority (most important first). The UI shows 3 at a time and slides in the next from the queue when ${firstName} dismisses one, so MORE entries = a longer "important inbox" he can work through without breaking flow. Skip newsletters, FYIs, anything where someone else is the doer. If only 2 things truly need a reply today, return 2 — don't pad.
