...
function decodeMimeHeader(value) {
  ...
  // Pass 2 — fix UTF-8 mojibake (Windows-1252 misinterpretation).
  // The typical pattern: raw UTF-8 bytes for non-ASCII text got pumped
  // through a Win-1252 decoder somewhere upstream, so each byte became
  // its Win-1252 character. Bytes 0x80-0x9F in Win-1252 map to Unicode
  // codepoints OUTSIDE Latin-1 ("„" U+201E, "Œ" U+0152, "…" U+2026,
  // etc.) — so `Buffer.from(s, "latin1")` truncates them and breaks.
  // We rebuild the byte stream using the Win-1252 reverse map, then
  // decode those bytes as UTF-8. Accept only if the result has no
  // replacement chars and strictly fewer high-bit chars.
  if (/[À-ÿ€‚ƒ„…†‡ˆ‰Š‹ŒŽ‘’“”•–—˜™š›œžŸ]/.test(out)) {
    try {
      const bytes = encodeAsWin1252(out);
      const fixed = Buffer.from(bytes).toString("utf-8");
      if (!fixed.includes("�")) {
        const before = (out.match(/[\x80-￿]/g) || []).length;
        const after = (fixed.match(/[\x80-￿]/g) || []).length;
...
// Reverse map for the 27 chars in the Windows-1252 0x80-0x9F range that
// map to Unicode codepoints outside Latin-1. Used to undo "UTF-8 bytes
// shown as Win-1252 chars" mojibake.
...
function encodeAsWin1252(str) {
  const bytes = [];
  for (let i = 0; i < str.length; i++) {
    const code = str.charCodeAt(i);
    if (code <= 0xFF) {
      // ASCII + Latin-1 range — direct passthrough (Win-1252 matches
      // Latin-1 outside the 0x80-0x9F window).
      bytes.push(code);
    } else if (WIN1252_REVERSE[code] !== undefined) {
      bytes.push(WIN1252_REVERSE[code]);
    } else {
      // Chars not in either map — string isn't pure mojibake, bail and
      // let the caller's safety check reject the fix.
      throw new Error("non-win1252 char at " + i);
    }
  }
