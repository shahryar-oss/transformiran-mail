...
// Hard timeouts on every Gmail call. Without these, a single hung
// request (Google network blip, throttle with no response) would
// freeze the await forever — and because the backfill worker guards
// each cycle with a `running` flag, ONE hung call permanently wedged
// the whole worker (incident 2026-05-27: Pia's job stuck `pending`
// 11h, worker dead). Mirror of the withTimeout pattern in
// inbox_cache.js.
const GMAIL_LIST_TIMEOUT_MS = 15000;
const GMAIL_GET_TIMEOUT_MS  = 10000;
...
function buildGmailQuery(query) {
  ...
  // Empty query (Delta asking "what did I get today / what's recent") →
  // the live inbox, newest-first. Lets the recency path surface
  // just-arrived mail the local index hasn't mirrored yet.
  return out.length ? out.join(" ") : "in:inbox";
}

async function searchViaGmail(userId, query, { limit = 25, skipIds = null } = {}) {
  const creds = await loadGoogleCreds(userId);
  if (!creds) return [];
  const oauth = authedClientFromTokens(creds);
  const g = google.gmail({ version: "v1", auth: oauth });
  ...
  let ids = (listRes.data.messages || []).map((m) => m.id);
  if (!ids.length) return [];

  // Skip IDs the caller already has (from the local index) so we only
  // pay metadata-fetch latency for genuinely-fresh messages. When the
  // index is already current this leaves nothing to fetch — the whole
  // call costs just the single list request above.
  if (skipIds && skipIds.size) {
    ids = ids.filter((id) => !skipIds.has(id));
    if (!ids.length) return [];
  }
...
// Public entry-point used by Delta's search_inbox tool.
//
// The inbox the user SEES is Gmail; the local index can LAG — a
// just-arrived email isn't mirrored into gmail_messages_indexed until
// the next inbox sync. The old behaviour only consulted Gmail when the
// local index returned ZERO rows, so a query like `from:lana` returned
// OLD indexed Lana mail (non-empty) and we'd never surface today's
// email — Delta then told the user it was "not available" for mail they
// were literally looking at in the inbox.
//
// Now we ALWAYS ask Gmail too (its q= covers the full corpus + body
// text, newest-first) and merge with the local index. To keep latency
// low when the index is already fresh, we hand Gmail the IDs we already
// have so it skips their metadata fetches — when nothing is newer, the
// extra cost is a single Gmail list call (~200ms). When fresh mail
// exists, only those few messages are fetched and merged in, newest-first.
async function searchInboxWithFallback(userId, query, { limit = 25 } = {}) {
  const lim = Math.min(50, Math.max(1, Number(limit) || 25));
  const local = await searchIndexed(userId, query, { limit: lim }).catch((err) => {
    console.warn("[searchInboxWithFallback] local search failed:", err.message);
    return [];
  });

  const knownIds = new Set(local.map((r) => r.message_id));
  const remote = await searchViaGmail(userId, query, { limit: lim, skipIds: knownIds })
    .catch((err) => {
      console.warn("[searchInboxWithFallback] gmail lookup failed:", err.message);
      return [];
    });

  if (!remote.length) return local;
  if (!local.length) {
    console.log(`[search_inbox] local=0 → gmail q="${String(query).slice(0,80)}" returned ${remote.length}`);
    return remote;
  }

  // Merge the fresh Gmail-only rows with the local index, newest-first,
  // so a just-arrived email floats to the top. remote and local are
  // disjoint by construction (skipIds removed the overlap), so no dedupe
  // pass is needed.
  console.log(`[search_inbox] merged ${remote.length} fresh gmail row(s) with ${local.length} local for q="${String(query).slice(0,80)}"`);
  return remote
    .concat(local)
    .sort((a, b) => (Number(b.internal_date) || 0) - (Number(a.internal_date) || 0))
    .slice(0, lim);
}
