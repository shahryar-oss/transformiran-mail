...
4. **VIP-sender bias is conditional.** When mail is from Lana Silk / Lazarus
   Yeghnazar / Maggie Yeghnazar / Pia van Belen / Lauren / Barmak / Remco /
   Simon / any donor mentioning gifts:
...
function buildBatchPrompt(messages, user) {
...
async function classifyBatch(messages, user) {
  if (!messages.length) return [];
  const r = await client().messages.create({
    model: CLASSIFIER_MODEL,
    max_tokens: 1500,
    system: buildClassifySystem(user),
    messages: [{ role: "user", content: buildBatchPrompt(messages, user) }],
  });
...
async function classifyForUser(user, messages, opts = {}) {
  ...
  for (let i = 0; i < batches.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(
      slice.map((b) =>
        classifyBatch(b, userObj).catch((err) => {
