...
- NEVER use em-dashes ("—") or en-dashes ("–") in label or draft_body. The user has explicitly banned them as an AI tell. Use a comma, period, parentheses, or colon instead. Reread each chip before returning it and rewrite any line that contains "—" or "–".
- Plain hyphen-minus "-" is fine for compound modifiers ("year-end", "follow-up") but NOT as a dash substitute between clauses.
  ...
  try {
    ...
    const sr = Array.isArray(obj.smart_replies)
      ? obj.smart_replies
          ...
          .slice(0, 4)
          .map((x) => ({
            // Scrub em/en-dashes from both label and draft_body. User
            // banned them as an AI tell.
            label: scrubDashes((x.label || "").trim()).slice(0, 28) || scrubDashes(x.draft_body).slice(0, 24),
            draft_body: scrubDashes(x.draft_body.trim()).slice(0, 600),
...
// them as an AI tell. Mirror of the helper in assistant.js — duplicated
// here so this module doesn't depend on assistant.js (would be a cycle).
