...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      "Uses Slack's native search.messages via the user's token (best relevance, returns permalinks, sees DMs + private channels). Falls back to local Postgres if no user token. " +
      "" +
      "QUERY CONSTRUCTION — pick the right operator for the user's intent: " +
      "" +
      "FROM a person:       from:@firstname    (e.g. 'from:@simon budget')   ← messages SENT BY that person, anywhere " +
      "TO/WITH a person:    with:@firstname    (e.g. 'with:@simon')          ← messages IN your DM thread with them (most useful for 'show me my conversation with Simon') " +
      ...
      "" +
      "Trigger phrases → query mappings: " +
      "'find my conversation with Simon'             → with:@simon " +
      "'what did Simon say last week'                → from:@simon after:2026-05-18 " +
      "'find the budget discussion in #operations'   → in:#operations budget " +
      "'pull the PDF Pia shared'                     → from:@pia has:link  (or use file_id afterwards) " +
      "" +
      "When in doubt — combine. 'with:@simon Tehran' beats just 'simon' which only finds messages mentioning the word 'simon'.",
    input_schema: {
      type: "object",
      properties: {
        query: {
