  ...
  if (name === "propose_calendar_event") {
    try {
      ...
      if (calPrefs && Array.isArray(calPrefs.focusBlocks) && calPrefs.focusBlocks.length && !input.force) {
        const startDay = startDt.getDay();          // 0=Sun…6=Sat (local time)
      ...
      const savedLink = calPrefs && calPrefs.defaultMeetingLink;
      const savedActive = !!(savedLink && calPrefs.defaultMeetingProvider !== "none");
      ...
      if (location && /^https?:\/\//i.test(location)) {
        ...
        })[calPrefs && calPrefs.defaultMeetingProvider] || "Video meeting";
    ...
    try {
      ...
      return {
        ...
        draft: {
          ...
          // Carry the reply mode so the client composer re-opens in the
          // SAME mode (reply vs reply-all) instead of defaulting to
          // reply-only and dropping the all-recipients Cc.
          mode: input.reply_mode === "reply-all" ? "reply-all" : "reply",
        },
