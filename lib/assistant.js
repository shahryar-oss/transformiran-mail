...
5. Reply format (STRICT — Finance Delta renders this verbatim):
   ...
   - For each match (max 3): one line —
     ...
     specifies. If the email lists per-project amounts (Pearl €X,
     Apologetics €Y, Media €Z, …), QUOTE THEM ALL VERBATIM, one
     line per project, indented under the sender line. Currency
     symbols and exact decimals matter — Finance Delta will use
     these numbers to populate the dashboard's allocation fields.
     Do NOT summarise these as "the email breaks down the funds"
     — Finance Delta needs the actual figures. After the
     per-project lines, one closing indented sentence summarising
     intent (restricted/unrestricted, sender, fund instructions).
