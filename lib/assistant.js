...
// Build a Gmail/Outlook-style quoted-history block from the message
// being replied to. Format:
//
//   <blank line>
//   On Thu, 22 May 2026 11:39:50 +0000, Ilse Visser <ilse@…> wrote:
//   <blank line>
//   > Dear Shahryar,
//   > How are you?
//   > …
//
// Returns "" if there's nothing to quote. Caps the quoted body at
// 50KB so a 200-message chain doesn't blow up the message size.
// Quoted-original tails INSIDE the parent body are kept — that's the
// whole point of preserving thread history.
function buildQuotedHistoryText(parentMsg) {
  if (!parentMsg) return "";
  const text = parentMsg.bodyText || parentMsg.snippet || "";
  if (!text) return "";
  const from = parentMsg.from || "(unknown sender)";
  const date = parentMsg.date || "";
  const capped = String(text).slice(0, 50000);
  const quotedLines = capped.split(/\r?\n/).map((l) => `> ${l}`).join("\n");
...
`.trim();
  ...
  // out with only the new prose, so recipients had zero context. The
  // attribution + ">" prefixed lines match the standard format. The
