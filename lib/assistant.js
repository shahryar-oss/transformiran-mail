...
5. Reply format (STRICT — Finance Delta renders this verbatim):
   ...
   - For each match (max 3): one line —
     ...
     — then a summary block with EVERY allocation detail the email
     specifies. SCAN THE FULL BODY (include_body=true returns it)
     for ALL euro / dollar / pound figures associated with a project,
     fund, or category. If the email lists per-project amounts
     (Pearl €X, Apologetics €Y, Media €Z, Christmas Appeal €…,
     Freedom in Christ €…, Bible Translation €…), QUOTE THEM ALL
     VERBATIM, one line per item, indented under the sender line.
     INCLUDE EVERY ENTRY — even small ones like €7.58. Currency
     symbols and exact decimals matter — Finance Delta will use
     these numbers to populate the dashboard's allocation fields.
     If the email has BOTH a top-level summary (e.g. "Media:
     €17,205.58, IR: €3,291.00") AND a granular per-project split
     (Pearl, Apologetics, …), include the GRANULAR split — that's
     what Finance needs for project codes. Do NOT summarise these
     as "the email breaks down the funds" — Finance Delta needs
     the actual figures. After the per-project lines, one closing
     indented sentence summarising intent (restricted/unrestricted,
     sender, fund instructions).
   ...
   Example:
     ...
     FROM Simon Baynham · "PAYMENT TO ST 222 NL" · 2026-05-19
       ...
       Simon confirms a £18,250 (€20,496.58) payment to the ABNA
       account; funds are restricted with the per-project breakdown
       above.
     Confidence: high
            ...
            slice.map((r) =>
              g.users.messages
                ...
                .then((res) => {
                  ...
                  return text.slice(0, 8000); // cap per-message size
                })
                .catch(() => "")
            )
          );
