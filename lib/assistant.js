...
async function draftReply({ user, openMessageId, instructions }) {
  ...
  // Pull past sent emails to the same recipient — Delta uses these to match
  // the user's actual voice with this person. The "Found N writing examples"
  // feature.
  let styleExamples = [];
  let recipientEmail = "";
  try {
    ...
    if (creds) {
      ...
      const r = await style.findExamplesTo(oauth, replyTo, 10);
      styleExamples = r.examples;
      recipientEmail = r.recipient;
    }
  } catch (err) {
    console.warn("[draftReply] style fetch failed:", err.message);
  }
...
- Include a sign-off ending with the user's first name (or whatever
  pattern the examples above show).
...
module.exports = { chat, draftReply, buildContext, buildSystemPrompt, CHAT_MODEL, MODELS, resolveModel };
