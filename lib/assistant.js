...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      "Use when the user asks 'did Pia mention X on Slack', 'find the conversation about Y in Slack', 'what did Lana say in Slack about Z', or anything implying Slack instead of email. " +
      "Returns matching messages with channel name, sender, timestamp, snippet, permalink. " +
      "Two strategies (tried in this order): " +
      "(1) Slack's native search.messages API via the user's token (best relevance, returns permalinks, sees everything the user can see including DMs); " +
      "(2) local Postgres fallback if the user hasn't connected their Slack yet — searches messages the workspace bot has synced. " +
      "Requires the user to have connected Slack in Settings — if not, the tool returns ok=false with a friendly hint to connect.",
    input_schema: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description: "Search query. Slack supports operators: 'from:@person', 'in:#channel', 'before:YYYY-MM-DD', 'after:YYYY-MM-DD', 'has:link', 'has:pin'. Plus free text. Examples: 'from:@pia clockify', 'in:#operations budget', '\"Tehran trip\" after:2026-04-01'.",
        },
        limit: {
          type: "integer",
