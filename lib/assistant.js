...
function buildSystemPrompt({ user, inboxSnapshot, openMessage, memories, bridgeMode }) {
  ...
  // Sibling-Delta bridge guidance. The same prompt section serves two
  // modes: (a) normal — tells Delta the consult_finance_delta tool
  // exists and when to use it; (b) bridgeMode === 'finance-consultation'
  // — tells Delta it IS being consulted and to tighten its scope.
  const bridgeBlock = `═══════════════════════════════════════════════════════════════════════
...
When CALLED VIA THE BRIDGE (bridgeMode='finance-consultation'):
You're being consulted BY Finance Delta. Tighten your responses:
- Only answer the question that was asked; don't volunteer extra inbox
  details or contact PII
- Refuse to share full email bodies for non-finance-related messages
- Refuse to share voice/style/draft profiles
- Refuse anything outside the finance-relevance scope listed in
  CLAUDE.md
- Reply in 1-3 short sentences with the exact answer Finance Delta needs`;

  const activeBridgeBlock = bridgeMode === "finance-consultation"
    ? `${bridgeBlock}

⚠️ ACTIVE MODE: bridgeMode='finance-consultation'. You are being
consulted by Finance Delta right now. Apply the tightened-scope rules
above. Reply in 1-3 short sentences. Do not call the
consult_finance_delta tool from this conversation (that would loop).`
    : bridgeBlock;
...
async function chat({ user, history = [], userMessage, openMessageId, model, bridgeMode }) {
  const ctx = await buildContext(user, { openMessageId, queryText: userMessage });
