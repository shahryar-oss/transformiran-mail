...
// People whose unanswered emails should be flagged as HIGH PRIORITY
// during the guided inbox-organize routine. Matched by case-insensitive
// substring against either the sender display name or the email address.
const VIP_SENDERS = [
  { name: "Lana Silk",        emails: ["lana@transformiran.com"] },
  { name: "Lazarus Yeghnazar", emails: ["lazarus@transformiran.com"] },
  { name: "Maggie Yeghnazar", emails: ["maggie@transformiran.com"] },
  { name: "Pia van Belen",    emails: ["pia@transformiran.com"] },
  { name: "Lauren",           emails: [] },
];

function isVipSender(fromHeader) {
  if (!fromHeader) return false;
  const raw = String(fromHeader).toLowerCase();
  for (const vip of VIP_SENDERS) {
    if (vip.emails.some((e) => raw.includes(e.toLowerCase()))) return true;
    if (raw.includes(vip.name.toLowerCase())) return true;
  }
  return false;
}
...
async function executeTool(name, input, { user, ctx }) {
  if (name === "remember") {
  ...
  if (name === "create_task") {
    try {
      ...
      return {
        ...
        summary: `Added task: ${row.title}${listLabel}${dueLabel}${flags}`,
      };
    } catch (err) {
      return { ok: false, error: err.message || String(err) };
    }
...
async function chat({ user, history = [], userMessage, openMessageId, model }) {
  ...
  for (let hop = 0; hop < 5; hop++) {
    ...
    for (const block of toolUseBlocks) {
      try {
        const out = await executeTool(block.name, block.input || {}, { user, ctx });
...
async function draftReply({ user, openMessageId, instructions }) {
  ...
  // Gmail signature. Both go into Delta's context.
  ...
  try {
    ...
    if (creds) {
      ...
      const [styleResult, sig] = await Promise.all([
...
────────────────────────────────────
` : `No past emails to this recipient were found. Default to the user's
...
// =============================================================================
async function buildInboxRoutine(user) {
  const { pool } = require("./db");
  const ctx = await buildContext(user, {});
  const messages = ctx.inboxSnapshot || [];
  if (!messages.length) return [];
...
module.exports = { chat, draftReply, buildContext, buildSystemPrompt, buildCleanupProposal, buildInboxRoutine, CHAT_MODEL, MODELS, resolveModel };
