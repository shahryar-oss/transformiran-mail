...
`.trim()
  ...
  → If the user pastes a different URL in chat ("set up with https://zoom.us/j/9999"), pass that exact pasted URL as location. Otherwise leave location empty — the saved default will be used.`
    ...
    const onBehalfLine = assistsList.length
      ? `\n- SCHEDULING ON BEHALF OF: you set up meetings for ${assistsList.join(", ")}. When a meeting you're arranging INCLUDES one of them, THEY are the host:
  → The meeting uses the HOST's own saved meeting link (their personal Zoom/Meet), attached automatically by the backend — never your own, never invented.
  → Do NOT add yourself (${(user && user.email) || "the assistant"}) as an attendee. You're arranging the meeting between the host and the other person, not joining it. List only the host + the other party.
  → Refer to it as the host's meeting (e.g. "set up Shahryar's Zoom with John"), not yours.`
      : "";
    calendarPrefsBlock = `
...
async function executeTool(name, input, { user, ctx, userMessage, bridgeMode }) {
  ...
  if (name === "propose_calendar_event") {
    try {
      ...
      // EA / on-behalf-of resolution. If the current user assists one of the
      // attendees (e.g. Pia scheduling for Shahryar), that principal HOSTS the
      // meeting: use HIS saved link, drop the EA from the invite, set him as
      // organizer, and check conflicts against HIS calendar — not the EA's.
      ...
      let schedPrefs = calPrefs; // whose meeting link applies (host's in EA mode)
      if (host) {
        attendees = attendees.filter(a => a !== userEmail);
        if (!attendees.includes(host)) attendees.push(host);
        organizerEmail = host;
      ...
      // Skipped in EA mode — the EA's own focus blocks don't apply to a
      // meeting hosted by the principal (the freebusy check below covers
      // the host's real conflicts).
      if (!host && calPrefs && Array.isArray(calPrefs.focusBlocks) && calPrefs.focusBlocks.length && !input.force) {
      ...
      // Conflict check ±2h around the proposed slot. In EA mode we check the
      // HOST's calendar via freebusy (the EA's own calendar is irrelevant);
      // otherwise the current user's own events.
      let conflicts = [];
      try {
        ...
        if (host) {
          const { google } = require("googleapis");
          const creds = await loadGoogleCreds(user.id);
          const fbCal = google.calendar({ version: "v3", auth: authedClientFromTokens(creds) });
          const fb = await fbCal.freebusy.query({
            requestBody: { timeMin: windowStart, timeMax: windowEnd, items: [{ id: host }] },
          });
          const busy = (((fb.data.calendars || {})[host]) || {}).busy || [];
          conflicts = busy
            .filter(b => new Date(b.start).getTime() < +endDt && new Date(b.end).getTime() > +startDt)
            .map(b => ({ id: null, summary: "(busy)", start: b.start, end: b.end }));
        } else {
      ...
      return {
        ...
        proposed: {
          ...
          attendees,
          organizerEmail,
          onBehalfOf: host || null,
          location: location || null,
          description: description || null,
          calendarId: host || "primary",
        },
        attachedLink,
        conflicts,
        summary: conflicts.length
          ? `Conflict — ${conflicts.length} event${conflicts.length === 1 ? "" : "s"} already in ${host ? "the host's" : "that"} window.`
          : `Slot looks free.`,
      };
    } catch (err) {
  ...
  if (name === "create_calendar_event") {
    try {
      ...
      // EA / on-behalf-of: if the current user assists one of the attendees,
      // that principal hosts — use HIS saved link, drop the EA from the invite,
      // and create the event on HIS calendar. Falls back to the EA's own
      // calendar if she has no write access to his (she's still the organizer
      // but never an attendee).
      const userEmail = (user.email || "").toLowerCase();
      let attendees = Array.isArray(input.attendees)
        ? input.attendees.filter(a => typeof a === "string" && a.includes("@")).map(a => a.toLowerCase().trim())
        : [];
      ...
      const targetCalendarId = host || input.calendarId || "primary";
      let event;
      try {
        event = await calendarLib.createEvent(user.id, { calendarId: targetCalendarId, ...eventBody });
      } catch (e) {
        // No write access to the host's calendar → create on the EA's own
        // calendar instead (host + other party still invited; EA not added).
        if (host) {
          console.warn(`[calendar] EA create on host calendar ${host} failed (${e.message}); falling back to primary.`);
          event = await calendarLib.createEvent(user.id, { calendarId: "primary", ...eventBody });
        } else {
          throw e;
        }
      }
  ...
  if (name === "find_meeting_time") {
    try {
      ...
      const inAttendees = Array.isArray(input.attendees)
        ? input.attendees.filter(a => typeof a === "string" && a.includes("@")).map(a => a.toLowerCase().trim())
        : [];
      // EA / on-behalf-of: if the current user assists one of the requested
      // people, find a slot that works for the HOST + the others — NOT the EA.
      // The EA isn't attending, so her calendar must not constrain the search.
      const assists = effectiveAssists(userEmail, calPrefs);
      const host = resolveMeetingHost(userEmail, inAttendees, assists);
      const attendees = host
        ? Array.from(new Set(inAttendees.filter(a => a !== userEmail).concat(host))).filter(Boolean)
        : Array.from(new Set([userEmail, ...inAttendees])).filter(Boolean);
