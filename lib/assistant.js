...
`.trim()
  ...
  if (cal) {
    const linkLine = cal.defaultMeetingLink
      ? `- Default meeting link: ${cal.defaultMeetingLink} (provider: ${cal.defaultMeetingProvider || "zoom"}). ALWAYS attach this URL to any video meeting you create or propose, both in the calendar event location field AND in the event description. The user shouldn't have to ask.`
      : `- Default meeting link: NOT configured. When you create a video meeting, ask the user once whether to attach Zoom/Meet/Teams + offer to save it as default.`;
...
`.trim();
  ...
  return [IDENTITY_BASE, tierBlock, ORG_CONTEXT, userBlock, memoryBlock, calendarPrefsBlock, inboxBlock, openBlock, activeBridgeBlock]
    .filter(Boolean)
    .join("\n\n────────────────────────────────────\n\n");
}
...
const TOOLS = [
    ...
    input_schema: {
      ...
      properties: {
        ...
        reply_mode: {
          ...
          description:
            "Optional. 'reply' (default) — to the original sender only. 'reply-all' — to the sender + everyone on the original To/Cc (excluding the user themselves). Use 'reply-all' when the user says 'reply to everyone', 'reply all', 'cc the others', or when the conversation is clearly group-oriented.",
        },
        to_override: {
          type: "string",
          description:
  ...
  },
  {
    ...
    description:
      ...
      "If the user has a default meeting link configured (see USER CALENDAR PREFERENCES above), " +
      "this tool will AUTOMATICALLY attach it to the event — DON'T ask separately and DON'T leave " +
      "the meeting link out of the preview. " +
  ...
  },
  {
    ...
    description:
      ...
      "If the user has a default meeting link configured (see USER CALENDAR PREFERENCES above), " +
      "this tool will AUTOMATICALLY attach it to the event's location + description — DON'T ask " +
      "the user for the link separately.",
    input_schema: {
      type: "object",
      properties: {
        summary: { type: "string" },
...
async function executeTool(name, input, { user, ctx, userMessage, bridgeMode }) {
  ...
  if (name === "propose_calendar_event") {
    try {
      ...
      // Phase 5.CK — auto-attach the user's default meeting link to
      // the proposal so the preview card already shows the join URL
      // before the user clicks Create event.
      let location = (input.location || "").trim();
      let description = (input.description || "").trim();
      let attachedLink = null;
      const calPrefs = ctx && ctx.userPrefs && ctx.userPrefs.calendar;
      if (!location && calPrefs && calPrefs.defaultMeetingLink && calPrefs.defaultMeetingProvider !== "none") {
        location = calPrefs.defaultMeetingLink;
        attachedLink = calPrefs.defaultMeetingLink;
        const providerLabel = ({
          zoom: "Zoom", meet: "Google Meet", teams: "Microsoft Teams", whereby: "Whereby",
        })[calPrefs.defaultMeetingProvider] || "Video meeting";
        const meetingNote = `${providerLabel}: ${calPrefs.defaultMeetingLink}`;
        description = description ? `${description}\n\n${meetingNote}` : meetingNote;
      }

      const attendees = Array.isArray(input.attendees)
        ? input.attendees.filter(a => typeof a === "string" && a.includes("@")).map(a => a.toLowerCase().trim()).slice(0, 50)
        : [];
      const userEmail = (user.email || "").toLowerCase();

      // Conflict check ±2h around the proposed slot.
      let conflicts = [];
      try {
  ...
  if (name === "create_calendar_event") {
    try {
      ...
      // Phase 5.CK — auto-attach the user's default meeting link
      // (location + description) unless an explicit location was
      // already passed. So "create a Zoom with Sam tomorrow at 3pm"
      // lands an event with the join URL already populated.
      let location = (input.location || "").trim();
      let description = (input.description || "").trim();
      let attachedLink = null;
      const calPrefs = ctx && ctx.userPrefs && ctx.userPrefs.calendar;
      ...
      const attendees = Array.isArray(input.attendees)
        ? input.attendees.filter(a => typeof a === "string" && a.includes("@")).slice(0, 50)
        : [];
  ...
  if (name === "find_meeting_time") {
    try {
      const userEmail = (user.email || "").toLowerCase();
      const inAttendees = Array.isArray(input.attendees) ? input.attendees.filter(a => typeof a === "string" && a.includes("@")) : [];
      const attendees = Array.from(new Set([userEmail, ...inAttendees.map(a => a.toLowerCase().trim())])).filter(Boolean);
  ...
  if (name === "draft_reply") {
    ...
    try {
      const result = await draftReply({
        ...
        mode: input.reply_mode === "reply-all" ? "reply-all" : "reply",
      });
